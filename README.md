...
- 🎬 **FFmpeg工具集** - 音视频格式转换、流媒体服务器、自动下载管理
...
强大的FFmpeg音视频处理工具，支持格式转换、流媒体服务和自动管理。
...
核心功能:
  -w, --wizard             启动交互式转换向导
  --status                 显示FFmpeg安装状态和版本信息
  --update                 更新FFmpeg到最新版本
  --install                自动下载并安装FFmpeg

转换选项:
  --convert <input>        转换指定的视频/音频文件
  -o, --output <output>    指定输出文件路径
  --format <format>        输出格式 (mp4,avi,mkv,webm,flv,mp3,wav等)
  --quality <level>        质量等级 (1-高质量 2-平衡 3-移动 4-高压缩)
  --resolution <size>      指定分辨率 (如: 1920x1080, 1280x720)
  --bitrate <rate>         指定视频码率 (如: 4000k)
  --audio-bitrate <rate>   指定音频码率 (如: 128k)

流媒体选项:
  --stream                 启动流媒体服务器模式
  --stream-type <type>     流媒体类型 (rtmp,hls,flv,dash)

批量处理:
  --batch <dir>            批量转换指定目录下的文件
  --extract-audio          从视频中提取音频
  --compress               启用压缩模式，减小文件大小
```

**功能特性:**
- 🚀 **自动FFmpeg管理** - 自动下载、安装、更新FFmpeg
- 🎬 **格式转换** - 支持几十种音视频格式互转
- 📡 **流媒体服务** - RTMP/HLS/HTTP-FLV/DASH流服务器
- 📊 **实时进度** - 详细的转换进度和性能监控
- 🎯 **智能预设** - 针对不同场景的质量优化预设
...
# 检查FFmpeg状态
awesome-tools ffmpeg --status

# 视频格式转换
awesome-tools ffmpeg --convert video.avi --format mp4 --quality 2

# 提取音频
awesome-tools ffmpeg --convert video.mp4 --extract-audio --format mp3

# 批量转换
awesome-tools ffmpeg --batch ./videos --format mp4

# 启动RTMP流服务器
awesome-tools ffmpeg --stream --stream-type rtmp

# 视频压缩
awesome-tools ffmpeg --convert large.mp4 --compress --resolution 1280x720
```
