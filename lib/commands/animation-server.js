...
class AnimationServer {
  ...
  handleMessage(clientId, data) {
    ...
    try {
      ...
      switch (message.type) {
        ...
        case 'scene_metadata_update':
          this.handleSceneMetadataUpdate(client, message);
          break;
  ...
  handleInspectResponse(client, message) {
    console.log(`[Animation-Server] 🔬 收到检查响应 (客户端 #${client.id})`);

    const { requestId, result, data } = message;
    console.log(`[Animation-Server] 响应详情: requestId=${requestId}, hasResult=${!!result}, hasData=${!!data}`);

    // 查找对应的待处理请求（支持新的pendingInspectRequests）
    const requests = this.pendingInspectRequests || this.pendingRequests;
    console.log(`[Animation-Server] 待处理请求数量: ${requests ? requests.size : 0}`);
    console.log(`[Animation-Server] 待处理请求ID列表: [${requests ? Array.from(requests.keys()).join(', ') : 'null'}]`);

    if (requests && requests.has(requestId)) {
      console.log(`[Animation-Server] ✅ 找到匹配的请求ID: ${requestId}`);
      ...
      try {
        ...
        console.log(`[Animation-Server] 处理响应数据: ${JSON.stringify(inspectData).substring(0, 200)}...`);
        ...
        if (!res.headersSent) {
          ...
          console.log(`[Animation-Server] ✅ HTTP响应已发送`);
        } else {
          console.log(`[Animation-Server] ⚠️ HTTP响应头已发送，跳过响应`);
        }

        if (resolve) {
          resolve(inspectData);
          console.log(`[Animation-Server] ✅ Promise已resolve`);
        }
      } catch (error) {
        console.error(`[Animation-Server] ❌ 处理检查响应失败:`, error);
        if (!res.headersSent) {
          res.writeHead(500);
          res.end(JSON.stringify({
            success: false,
      ...
      console.warn(`[Animation-Server] ⚠️ 未找到请求ID ${requestId} 的待处理请求`);
      console.log(`[Animation-Server] 当前待处理请求详情:`);
      if (requests) {
        requests.forEach((value, key) => {
          console.log(`[Animation-Server]   - ${key}: ${JSON.stringify({timestamp: value.timestamp})}`);
        });
      } else {
        console.log(`[Animation-Server]   requests对象为null`);
      }
    }
  }
   ...
   * 处理场景元数据更新
   */
  handleSceneMetadataUpdate(client, message) {
    const { uniqueId, ...metadataUpdates } = message.data || {};

    if (!uniqueId) {
      console.warn('⚠️ 场景元数据更新缺少uniqueId');
      return;
    }
    ...
    const scene = this.getSceneByUniqueId(uniqueId);
    if (!scene) {
      console.warn(`⚠️ 收到未知场景的元数据更新: ${uniqueId}`);
      return;
    }

    // 更新场景元数据
    const oldMetadata = { ...scene.metadata };
    scene.metadata = { ...scene.metadata, ...metadataUpdates };
    scene.lastMetadataUpdate = Date.now();

    // 记录更新日志
    console.log(`📝 场景元数据更新: ${scene.nickname} (${uniqueId})`);
    console.log(`   更新字段: ${Object.keys(metadataUpdates).join(', ')}`);

    // 如果启用MCP桥接，保存更新的数据
    if (this.options.enableMCPBridge) {
      this.saveMCPData('scenes', scene.uniqueId, scene);
    }

    // 响应元数据更新确认
    this.sendMessage(client.ws, {
      type: 'metadata_update_ack',
      uniqueId: uniqueId,
      timestamp: Date.now(),
  ...
  getAllScenes() {
    return Array.from(this.scenes.values()).map(scene => ({
      ...
      lastMetadataUpdate: scene.lastMetadataUpdate,
      heartbeatAge: Date.now() - (scene.lastHeartbeat || 0),
      metadataAge: scene.lastMetadataUpdate ? Date.now() - scene.lastMetadataUpdate : null,
  ...
  handleSceneInspectAPI(req, res) {
    ...
    req.on('end', () => {
      try {
        ...
        console.log(`[Animation-Server] 🔍 收到场景检查API请求:`, { components, detailed });
        ...
        console.log(`[Animation-Server] 找到 ${inspectorClients.length} 个场景检查器客户端`);
        ...
        console.log(`[Animation-Server] 生成请求ID: ${requestId}, 目标客户端: #${client.id}`);
        ...
        console.log(`[Animation-Server] 存储待处理请求: ${requestId}, 当前待处理数量: ${this.pendingRequests.size}`);
        ...
        console.log(`[Animation-Server] 发送检查命令到客户端 #${client.id}:`, command);
        client.ws.send(JSON.stringify(command));

        // 设置超时
        setTimeout(() => {
          if (this.pendingRequests.has(requestId)) {
            console.log(`[Animation-Server] ⏰ 请求超时: ${requestId}`);
            this.pendingRequests.delete(requestId);
            if (!res.headersSent) {
              res.writeHead(408);
              res.end(JSON.stringify({
        ...
        console.error(`[Animation-Server] API请求处理失败:`, error);
        res.writeHead(400);
        res.end(JSON.stringify({
          success: false,
          error: '请求格式错误',
