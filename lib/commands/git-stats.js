...
async function generateGitStats(options) {
  ...
  if (dailyStats.size > 0) {
    ...
    console.log('│ 日期       │ 提交数 │ 净增行 │ ' + '代码行数分布图'.padEnd(chartAndAuthorColWidth - 1) + '│');
    ...
    for (const dayStats of sortedDailyStats) {
      ...
      const chartAndAuthorContent = combinedContent.length > chartAndAuthorColWidth - 1 ? 
        combinedContent.substring(0, chartAndAuthorColWidth - 1) : 
        combinedContent.padEnd(chartAndAuthorColWidth - 1);
