...
const { VueRuntimeScanner } = require('../utils/runtime-scanner');
...
async function cleanDeadCode(options) {
  ...
  // 运行时扫描功能
  if (options.runtime) {
    console.log('🎯 启用运行时扫描功能...');
    const runtimeScanner = new VueRuntimeScanner();
    
    // 查找项目中的 main.js 文件
    const mainJSFiles = [];
    
    // 首先从已扫描的文件中查找
    files.filter(f => f.relativePath.includes('main.js') || f.relativePath.includes('main.ts')).forEach(f => mainJSFiles.push(f));
    
    // 额外检查常见的 main.js 文件位置
    const commonMainPaths = [
      'src/main.js',
      'src/main.ts',
      'main.js',
      'app.js',
    ...
    for (const mainPath of commonMainPaths) {
      const fullPath = path.join(projectDir, mainPath);
      if (fs.existsSync(fullPath)) {
        // 检查是否已经在列表中
        const exists = mainJSFiles.some(f => f.path === fullPath);
        if (!exists) {
          mainJSFiles.push({
            path: fullPath,
    ...
    if (mainJSFiles.length > 0) {
      console.log(`📄 找到 ${mainJSFiles.length} 个入口文件，正在注入跟踪脚本...`);
      mainJSFiles.forEach(file => {
        try {
          runtimeScanner.injectIntoMainJS(file.path);
        } catch (error) {
          console.warn(`⚠️  无法注入到 ${file.relativePath}: ${error.message}`);
        }
      });
      
      console.log('');
      console.log('🚀 运行时跟踪已设置完成！');
      console.log('💡 现在请重启你的 Vue 应用');
      console.log('💡 跟踪代码已集成到应用入口文件中');
      console.log('💡 使用数据将被自动收集并存储在浏览器中');
      console.log('💡 使用 --analyze-runtime 参数来分析收集的数据');
      console.log('');
      console.log('📝 使用说明:');
      console.log('   1. 在你的main.js中，用跟踪器包装createApp和router:');
      console.log('   2. const app = window.__vueUsageTracker.trackCreateApp(createApp)(App)');
      console.log('   3. const router = window.__vueUsageTracker.trackRouter(createRouter(...))');
      console.log('');
    } else {
      console.log('⚠️  未找到 main.js 文件，无法注入运行时跟踪脚本');
      console.log('💡 请确保项目包含 src/main.js 或其他入口文件');
    }
    
    return; // 运行时模式下，只进行注入不进行死代码分析
  }
  
  // 分析运行时数据
  if (options.analyzeRuntime) {
    console.log('📊 分析运行时使用数据...');
    const runtimeScanner = new VueRuntimeScanner();
    const usageData = runtimeScanner.analyzeUsageData(projectDir);
    runtimeScanner.generateUsageReport(usageData, projectDir);
    return;
  }
