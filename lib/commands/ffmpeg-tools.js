...
class FFmpegTools {
        ...
        return new Promise((resolve) => {
            ...
            ffmpeg.on('close', (code) => {
                ...
                if (code === 0) {
                    ...
                    // 播放模式下客户端断开时重新启动流
                    if (mode === '2') {
                        console.log(`🔄 重新开始播放: ${path.basename(inputSource)}`);
                        // 这里可以选择重新启动或者退出
                        console.log(`💡 提示: 如需重新播放，请重启RTMP服务器`);
                    }
                } else {
                    console.log(`\n⚠️  RTMP服务器退出 (退出码: ${code})`);
                }
                resolve();
            });
            ...
            ffmpeg.on('error', (error) => {
                ...
                if (errorMsg.includes('connection refused') || errorMsg.includes('broken pipe')) {
                    console.log(`\n📱 客户端连接断开`);
                    if (mode === '2') {
                        console.log(`💡 播放已暂停，等待新的客户端连接...`);
                    }
                } else {
    ...
    async startHLSServer() {
        ...
        if (videoFiles.length > 0) {
            ...
            inputFile = await this.askQuestion('输入文件或流地址: ');
        }
        ...
        return new Promise((resolve) => {
            ...
            ffmpeg.stderr.on('data', (data) => {
                ...
                if (!isGenerating && (output.includes('Opening') || output.includes('segment'))) {
                    ...
                    console.log('✅ HLS流生成中...');
                    console.log(`📁 输出目录: ${outputDir}`);
                    console.log(`🌐 HTTP服务器已启动: http://localhost:${httpPort}`);
                    console.log(`📺 播放地址: http://localhost:${httpPort}/stream.m3u8`);
                    console.log(`🎮 网页播放器: http://localhost:${httpPort}/player.html`);
                    console.log('\n💡 推荐使用网页播放器，支持更好的HLS播放体验\n');
                }
                
                // 显示进度
                const timeMatch = output.match(/time=(\d{2}):(\d{2}):(\d{2})/);
                if (timeMatch && isGenerating) {
                    const currentTime = parseInt(timeMatch[1]) * 3600 + 
                                      parseInt(timeMatch[2]) * 60 + 
                                      parseInt(timeMatch[3]);
    ...
    createHTTPServer(staticDir, port) {
        ...
        const server = http.createServer((req, res) => {
            ...
            if (parsedUrl.pathname === '/') {
                filePath = path.join(staticDir, 'stream.m3u8');
            }
