...
class FFmpegTools {
    ...
    async startWizard() {
        ...
        console.log('  10. 🎨 图片格式转换');
        console.log('  0. 退出\n');
        
        const choice = await this.askQuestion('请输入选项 (0-10): ');
        ...
        switch (choice) {
            ...
            case '10':
                return await this.imageProcessingWizard();
            case '0':
                console.log('👋 再见!');
                break;
            default:
...
<body>
     ...
     * 图片处理向导
     */
    async imageProcessingWizard() {
        console.log('\n🎨 图片格式转换向导');
        console.log('================================================================================');
        
        // 扫描当前目录的图片文件
        const imageFiles = this.scanImageFiles('.');
        
        if (imageFiles.length > 0) {
            console.log(`\n📁 检测到 ${imageFiles.length} 个图片文件:`);
        ...
        console.log('\n请选择您要执行的操作:');
        console.log('  1. 🔄 图片格式转换');
        console.log('  2. 📄 图片转Base64编码');
        console.log('  3. 🖼️ Base64转图片');
        console.log('  0. 返回主菜单\n');
        
        const choice = await this.askQuestion('请输入选项 (0-3): ');
        
        switch (choice) {
            case '1':
                return await this.imageFormatConversionWizard(imageFiles);
            case '2':
                return await this.imageToBase64Wizard(imageFiles);
            case '3':
                return await this.base64ToImageWizard();
            case '0':
                return await this.startWizard();
            default:
                console.log('❌ 无效选择');
     ...
     * 图片格式转换向导
     */
    async imageFormatConversionWizard(imageFiles) {
        console.log('\n🔄 图片格式转换');
        console.log('================================================================================');
        ...
        if (imageFiles.length > 0) {
            ...
            const choice = await this.askQuestion(`请选择 (1-${imageFiles.length}) 或输入自定义路径: `);
            const index = parseInt(choice) - 1;
            
            if (index >= 0 && index < imageFiles.length) {
                inputFile = imageFiles[index].path;
            } else {
                inputFile = choice;
            }
        } else {
            inputFile = await this.askQuestion('请输入图片文件路径: ');
        }
        ...
        console.log('\n选择输出格式:');
        console.log('  1. PNG - 无损压缩，支持透明背景');
        console.log('  2. JPEG - 有损压缩，文件较小');
        console.log('  3. WebP - 现代格式，体积小画质好');
        console.log('  4. GIF - 支持动画');
        console.log('  5. BMP - 无压缩格式');
        
        const formatChoice = await this.askQuestion('\n请选择格式 (1-5): ');
        const formats = { '1': 'png', '2': 'jpg', '3': 'webp', '4': 'gif', '5': 'bmp' };
        const outputFormat = formats[formatChoice];
        ...
        const outputFile = await this.askQuestion(`输出文件名 (默认: ${baseName}.${outputFormat}): `) || `${baseName}.${outputFormat}`;
        ...
        try {
            ...
            const another = await this.askQuestion('\n是否转换其他图片? (Y/n): ');
     ...
     * 图片转Base64向导
     */
    async imageToBase64Wizard(imageFiles) {
        console.log('\n📄 图片转Base64编码');
        console.log('================================================================================');
        ...
        const copyToClipboard = await this.askQuestion('\n是否复制Base64到剪贴板? (Y/n): ');
        const shouldCopy = copyToClipboard.toLowerCase() !== 'n';
        
        console.log('\n📄 开始转换...');
        
        try {
            await this.handleImageToBase64({
                imageToBase64: inputFile,
                clipboard: shouldCopy
            });
     ...
     * Base64转图片向导
     */
    async base64ToImageWizard() {
        console.log('\n🖼️ Base64转图片');
        console.log('================================================================================');
        
        console.log('\n输入Base64数据的方式:');
        console.log('  1. 从剪贴板粘贴');
        console.log('  2. 手动输入');
        console.log('  3. 从文件读取');
        
        const inputChoice = await this.askQuestion('\n请选择 (1-3): ');
        ...
        switch (inputChoice) {
            case '1':
                try {
                    const { default: clipboardy } = await import('clipboardy');
                    base64Data = await clipboardy.read();
                    console.log('✅ 已从剪贴板读取数据');
                } catch (error) {
                    console.log('❌ 读取剪贴板失败，请手动输入');
                    base64Data = await this.askQuestion('请输入Base64数据: ');
                }
                break;
            case '2':
                base64Data = await this.askQuestion('请输入Base64数据: ');
                break;
            case '3':
                const filePath = await this.askQuestion('请输入包含Base64数据的文件路径: ');
                try {
                    base64Data = fs.readFileSync(filePath, 'utf8').trim();
                    console.log('✅ 已从文件读取数据');
                } catch (error) {
                    console.log('❌ 读取文件失败');
                    return await this.base64ToImageWizard();
                }
        ...
        const outputName = await this.askQuestion('输出文件名 (默认: output.png): ') || 'output.png';
        
        console.log('\n🖼️ 开始转换...');
        
        try {
            await this.handleBase64ToImage({
