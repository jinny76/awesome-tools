/**
 * Dev Server 配置管理 - 基于文件IPC的持续Claude系统
 */
...
class DevServerConfig {
  constructor() {
    this.configDir = path.join(process.cwd(), '.dev-server');
   ...
   * 获取服务器的IPC目录路径
   */
  getServerIpcDir(name) {
    return path.join(this.configDir, 'ipc', name);
  }

  /**
   * 确保服务器IPC目录存在
   */
  ensureServerIpcDir(name) {
    const ipcDir = this.getServerIpcDir(name);
