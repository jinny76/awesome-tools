...
class AnimationServerManager {
  constructor(options = {}) {
    this.options = {
      ...
      maxReconnectAttempts: 2, // 最大重连次数改为2次
      ...
      try {
        ...
        this.websocket.onclose = (event) => {
          ...
          if (!this.isDestroyed && this.reconnectAttempts < this.options.maxReconnectAttempts) {
            ...
            console.log(`❌ [AnimationServer] 已达到最大重连次数(${this.options.maxReconnectAttempts})，停止重连`);
          }
        };
  ...
  parsePerformanceData(buffer) {
    ...
    return {
      fps: view.getFloat32(offset, true),
      frameTime: view.getFloat32((offset += 4), true),
      triangles: view.getUint32((offset += 4), true),
      vertices: view.getUint32((offset += 4), true),
      drawCalls: view.getUint32((offset += 4), true),
      memoryUsage: view.getFloat64((offset += 4), true)
    };
  }
  ...
  scheduleReconnect() {
    ...
    console.log(`[AnimationServer] ${delay}ms后尝试第${this.reconnectAttempts}次重连 (最多${this.options.maxReconnectAttempts}次)`);
