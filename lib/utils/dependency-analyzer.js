  ...
  for (const file of files) {
    ...
    for (const imp of analysis.imports) {
      ...
      if (imp.type === 'require-context') {
        resolvedPaths = resolveRequireContext(imp, file.path, projectDir, projectConfig);
      } else {
        resolvedPaths = resolveImportPath(imp.from, file.path, projectDir, projectConfig);
      }
...
// 解析require.context的路径
function resolveRequireContext(importInfo, fromFile, projectDir, projectConfig) {
  const resolvedPaths = [];
  const { metadata } = importInfo;
  ...
  // 解析目录路径
  let searchDir = metadata.directory;
  
  // 处理相对路径
  if (searchDir.startsWith('.')) {
    const fromDir = path.dirname(fromFile);
    searchDir = path.resolve(fromDir, searchDir);
  } else if (!path.isAbsolute(searchDir)) {
    // 可能是别名路径
    const aliasKey = Object.keys(projectConfig.alias).find(alias => 
      searchDir.startsWith(alias + '/') || searchDir === alias
    );
    
    if (aliasKey) {
      const aliasPath = projectConfig.alias[aliasKey];
      const remainingPath = searchDir.substring(aliasKey.length);
      searchDir = path.join(aliasPath, remainingPath);
    } else {
      // 相对于项目根目录
      searchDir = path.resolve(projectDir, searchDir);
    }
  }
  ...
  try {
    const regex = new RegExp(metadata.regex, metadata.flags);
    ...
    function scanDirectory(dir, currentDepth = 0) {
      ...
      for (const item of items) {
        ...
        if (stat.isDirectory() && metadata.recursive) {
    ...
    console.warn(`require.context路径解析错误: ${error.message}`);
  }
