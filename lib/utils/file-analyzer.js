...
function analyzeFileContent(content, file) {
  ...
  // 匹配require.context (webpack批量导入)
  const requireContextRegex = /require\.context\s*\(\s*['"`]([^'"`]+)['"`]\s*,\s*(true|false)\s*,\s*\/([^/]+)\/([gimuy]*)\s*\)/g;
  while ((match = requireContextRegex.exec(content)) !== null) {
    ...
    try {
      ...
      // 将require.context转换为通配符导入
      const contextPattern = `${directory}/${recursive ? '**/' : ''}*`;
      imports.push({
        from: contextPattern,
        type: 'require-context',
        items: [{ name: '*', type: 'context' }],
        metadata: {
          directory,
          recursive,
          pattern,
      ...
      console.warn(`无法解析require.context正则表达式: ${pattern}`);
    }
  }
...
async function scanSourceFiles(projectDir, options) {
  ...
  if (options.gitignore !== false) { // 默认使用gitignore，除非明确设置为false
    gitignorePatterns = parseGitignore(projectDir);
    if (gitignorePatterns.length > 0) {
      console.log(`📋 从.gitignore加载了 ${gitignorePatterns.length} 个排除规则`);
    }
  }
