...
class KingfisherSceneInspector {
  ...
  copySceneConfiguration(sourceUniqueId, targetUniqueId, configType = 'all') {
    if (!this.isConnected) {
      console.warn('未连接到动画服务器，无法执行配置复制');
      return null;
    }
    
    // 发送配置复制请求到服务器
    this.sendMessage({
      type: 'copy_scene_configuration',
      timestamp: Date.now(),
      data: {
        sourceUniqueId,
        targetUniqueId,
        configType,
        clientRequester: this.clientId
      }
    });
    
    console.log(`🔄 已请求配置复制: ${sourceUniqueId} -> ${targetUniqueId} (类型: ${configType})`);
    return true;
  }
    ...
    this.sceneWatcher = setInterval(() => {
      ...
      if (this.hasSceneStateChanged(lastSceneState, currentState)) {
        ...
        if (currentState.hasScene && currentState.sceneId !== this.currentSceneId) {
          ...
          if (!this.currentUniqueId || this.currentUniqueId.includes('_temp_')) {
            // 生成新的唯一标识
    ...
    try {
      ...
      if (performance) {
        ...
        if (performance.fps < 30) {
          health.issues.push('帧率过低 (< 30 FPS)');
          health.score -= 30;
        }
        
        if (performance.triangles > 2000000) {
          health.issues.push('三角形数量过多 (> 2M)');
          health.score -= 20;
        }
      }
      ...
      health.metrics.nodeCount = nodeCount;
      
      if (nodeCount > 10000) {
        health.issues.push('场景节点过多 (> 10K)');
        health.score -= 20;
      }

      // 检查场景是否就绪
      if (scene.isReady && !scene.isReady()) {
        health.issues.push('场景未就绪');
        health.score -= 50;
      }
